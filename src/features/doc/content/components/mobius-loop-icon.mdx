---
title: Mobius Loop Icon
description: Animated Mobius loop icon that morphs between circles and infinity shape.
image: /placeholder-assets/images/blog/mobius-loop-icon.webp
createdAt: 2026-05-30
updatedAt: 2026-05-30
---

<ComponentPreview
 name="mobius-loop-icon-demo"
 openInV0Url="https://placeholder.local/r/mobius-loop-icon.json"
/>

<DocTestimonial
 authorAvatar="https://unavatar.io/x/orcdev"
 authorName="OrcDev"
 authorTagline="Creator of 8bitcn.com"
 url="https://x.com/orcdev/status/2061155001444012321"
 quote="looks really nice!"
 date="2026-06-01"
/>

## Installation

<CodeTabs>

<TabsListInstallType />

<TabsContent value="cli">

```bash
npx shadcn@latest add @placeholder/mobius-loop-icon
```

</TabsContent>

<TabsContent value="manual">

<Steps>

<Step>Install the following dependencies</Step>

```bash
npm install motion
```

<Step>Copy and paste the following code into your project</Step>

<ComponentSource
 name="mobius-loop-icon"
 title="components/mobius-loop-icon.tsx"
/>

<Step>Update the import paths to match your project setup</Step>

</Steps>

</TabsContent>

</CodeTabs>

## Usage

```tsx
import { MobiusLoopIcon } from "@/components/mobius-loop-icon"
```

```tsx
<MobiusLoopIcon />
```
